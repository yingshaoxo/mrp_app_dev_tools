...
/* Access codes for ADP_ReadExt and ADPWriteExt, as RDI_Access.. */
#define ADPAccess_Data      0
#define ADPAccess_Data8     1
#define ADPAccess_Data16    2
#define ADPAccess_Data32    3
#define ADPAccess_Data64    4
#define ADPAccess_Code      8
#define ADPAccess_Code8     9  /* Not used at present */
#define ADPAccess_Code16    10
#define ADPAccess_Code32    11

#define ADP_ReadExt                     ADPREASON(CI_HADP,22)
/* ADP_ReadExt - may be unsupported
 * -----------
 * Summary: Request for a transer of memory contents from the target to the
 * debugger with access type specification.
 *
 * Arguments:
 * Send:   (word address, word nbytes, word access)
 * Return: (word status, word rnbytes [, bytes data])
 *
 * 'address' is the address from which memory transer should start.
 * 'nbytes' is the number of bytes to transfer.
 * 'access' is one of the ADPAccess_... codes above
 * 'status' is RDIError_NoError to indicate success, non-zero otherwise.
 * 'rnbytes' holds the number of requested bytes NOT read (i.e. zero
 * indicates success, non-zero indicates an error).
 * 'data' is the number of bytes requested minus 'rnbytes'.
...
#define ADP_WriteExt                    ADPREASON(CI_HADP,23)
/* ADP_WriteExt - may be unsupported
 * ------------
 * Summary: Request for a transfer of memory contents from the debugger to
 * the target with acccess type specification.
 *
 * Arguments:
 * Send:   (word address, word nbytes, word access, bytes data)
 * Return: (word status [, word rnbytes])
 *
 * 'address' is the address from which memory transer should start.
 * 'nbytes' is the number of bytes to transfer.
...
/* EOF angel_reasons.h (subset of adp_h) */
