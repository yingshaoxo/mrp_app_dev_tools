#ifndef __STD_RW_SSTREAM_CC__
#define __STD_RW_SSTREAM_CC__
...
namespace std {
  ...
  basic_stringbuf<charT, traits, Allocator>::overflow(int_type c)
  {
    ...
    if ( traits::eq_int_type(c,traits::eof()) ) var = traits::not_eof(c);
      ...
      var = sputc(_RWSTD_STATIC_CAST(char_type, c));
  ...
  {
    streamsize    newoff = _RWSTD_STATIC_CAST(streamsize, sp);
