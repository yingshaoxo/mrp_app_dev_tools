#ifndef __STD_RW_BITSET_CC__
#define __STD_RW_BITSET_CC__
...
//
  ...
  bitset<N>::to_string () const 
  {
    ...
    string s;
#endif
    for (long i = _RWSTD_STATIC_CAST(long, N - 1); i >= 0; --i)
      s.append(1, test(_RWSTD_STATIC_CAST(size_t, i)) ? '1' : '0');
    return s;
  }
...
//
  ...
  bitset<N>::count () const _RWSTD_THROW_SPEC_NULL
  {
    ...
    for (size_t i = 0; i < _NELEMENTS; i++)
    {
      ...
      sum += _RWSTD_STATIC_CAST(size_t, (t >> 6) + (t & 077) - 1);
    }
#else
    //
