#ifndef __STD_RW_RW_MONEY_CC__
#define __STD_RW_RW_MONEY_CC__
...
#ifdef ARM_MOVE_MONEYPUNCT_DATA_CTOR
...
template <class charT>
void moneypunct_data<charT>::__initfacetbase (const locale*) { _RWSTD_UNUSED(this); }
...
#ifdef ARM_NO_DATA_PTR_INIT
    result=new moneypunct_init<charT>;
#else
...
#ifdef ARM_NO_DATA_PTR_INIT
    _RW_STD::char_traits<char>::copy(result->gr_,init->gr_,sizeof(result->gr_) - 1); result->gr_[sizeof(result->gr_) - 1] = '\0';
    _RW_STD::char_traits<charT>::copy(result->cs_,cs_.c_str(),sizeof(result->cs_)/sizeof(charT) - 1); result->cs_[sizeof(result->cs_)/sizeof(charT) - 1] = charT('\0');
    _RW_STD::char_traits<charT>::copy(result->ps_,ps_.c_str(),sizeof(result->ps_)/sizeof(charT) - 1); result->ps_[sizeof(result->ps_)/sizeof(charT) - 1] = charT('\0');
    _RW_STD::char_traits<charT>::copy(result->ns_,ns_.c_str(),sizeof(result->ns_)/sizeof(charT) - 1); result->ns_[sizeof(result->ns_)/sizeof(charT) - 1] = charT('\0');
#else
    size_t n;
