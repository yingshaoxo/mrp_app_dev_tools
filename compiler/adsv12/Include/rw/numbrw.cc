#ifndef __STD_RW_RW_NUMBRW_CC__
#define __STD_RW_RW_NUMBRW_CC__
...
#ifdef ARM_MOVE_TIMEPUNCT_DATA_CTOR
    ...
    get_digit_groups (char *dpos)
{
  ...
  if (gpos==groups+sizeof groups)
    ...
    if (i >= CHAR_MAX)
      ...
      *gpos++=_RWSTD_STATIC_CAST(char, i);
...
void digit_writer<charT,OutputIterator>::put_digits (charT fill)
{
  ...
  charT separator = charT();  // initial value not used
