#ifndef __STD_RW_FSTREAM_CC__
#define __STD_RW_FSTREAM_CC__
...
#pragma option -w-par
   ...
   */
#ifndef _RWSTD_ANSI_FILEIO
  #define _RWSTD_IGNORE_ANSI_FILEIO(x) x
#else
  #define _RWSTD_IGNORE_ANSI_FILEIO(x)
#endif
  template<class charT, class traits>
  basic_filebuf<charT, traits> *
  basic_filebuf<charT, traits>::
  open(const char *s, ios_base::openmode mode, long _RWSTD_IGNORE_ANSI_FILEIO(protection))
  {
    long    m;
    ...
    if(__RWSTD::__rwOpenTable[m][0] == '\0') 
 #else
